...
export class KeyBinder {
    ...
    private timer_listener ?: NodeJS.Timeout
    ...
    private listenerCallback(e : Event){
            ...
            //if not just listened to clear current stroke
            if(!this.just_listened){
                this.current_stroke = []
                this.just_listened = true
            }
            // add meta key if no duplicate meta keys already present
            ...
            // add shift key if no duplicate shift keys already presents and key is part of the alphanumeric keys
            ...
            // because in keypress e.key name changes from `b` to `keyB` so we have to get the proper key name
            ...
            //if part of meta key and special function keys let it be set to lowercase 
            ...
            //adding the key to current key stroke 
            key  ? this.current_stroke.push(key) : null

            // console.log((alphanumeric.indexOf(`${e?.key}`) < 0 && this.current_stroke.filter(v => alphanumeric.indexOf(`${v}`) < 0 && this.meta_keys.indexOf(`${v}`) < 0 && v.length < 2).length == 1))
            //removing the shift key if the key is not part of the alphanumeric characters and character lenght is one or special function keys like (space, tab)
            if(alphanumeric.indexOf(`${e?.key}`) < 0 && this.current_stroke.filter(v => alphanumeric.indexOf(`${v}`) < 0 && this.meta_keys.indexOf(`${v}`) < 0 && v.length < 2).length == 1){
                // console.log('testing')
                this.current_stroke = this.current_stroke.filter(v => v.toLowerCase() != 'shift'.toLowerCase())
            }
            // changing any instance of control from e.key to ctrl and also removing any duplicate meta key like shift and ctrl
